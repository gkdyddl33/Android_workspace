...
//ListView가 사용할 컨트롤러 클래스. Swing에서의 TableModel과 그 역할이 같다
public class BoardAdapter extends BaseAdapter {
    ...
    // swing TableModel이제공하는 메서드와 동일하게 아래의모든 메서드는 호출 주체가 ListView이다.
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        View view = null;
        if(convertView==null){
            // 발급받은 적이 없다면(null), 새로 발급해주자
            // 아이템을 차지할 새로운 뷰를 생성해야 한다.
            // 워낙 디자인이 복잡하다보니, 순수 자바로 작성하지 말고, xml로부터 인플레이션 시키자
            // 인플레이션 시킬 때 주의점) 3번째 인수에 false를 부여해야, 인플레이션 결과의 최상단 컨테이너가 ListView가 아니게 반횐됨..
        ...
        // 인플레이션에 의해 반환된 최상위 컨테이너인 view는 자신이 보유한 자식요소에 접근할 수 있다.
