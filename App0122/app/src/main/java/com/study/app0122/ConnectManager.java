...
public class ConnectManager extends Thread{
    ...
    public int requestByGet(){ //Get방식으로 요청을 시도하는 메서드
        ...
        try{
            ...
            // 이 시점이 바로 통신이 완료된 시점이므로, 여기서 무언가를 해야 한다..
            // 그래서 화면 갱신은 여기서 진행을 해야 한다.. 즉, 메인액티비티가.printData(null);
            Log.d(TAG,"서버가 보낸 응답데이터는 : "+sb.toString());
            //mainActivity.printData(sb.toString());  // ----------> 사용자 정의 쓰레드는 디자인 접근 불가! onClick 디자인을 할 수 없다
                                                                 // ---------- 해결책? 디자인을 갱신해달라고 요청하면 된다.. 이때 요청을 받는 전담객체를 핸들러라 한다. Handler
            //mainActivity.handler.sendEmptyMessage(0);   // handler의 handlerMessage()를 호출하게됨..
            Message message = new Message();    // empty 메시지 객체 생성
            Bundle bundle = new Bundle();   // 꾸러미(담는것)
            bundle.putString("data",sb.toString());
            message.setData(bundle);

            mainActivity.handler.sendMessage(message); // 메시지 객체
// 아 된거같아요
