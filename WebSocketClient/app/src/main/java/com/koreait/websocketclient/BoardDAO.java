...
public class BoardDAO {
    ...
    //상세보기(메모리 상의 boardList 를 사용했기 때문에 필요가 없었다)
    ...
    public void edit(Board board) throws BoardUpdateException{
        ...
        try {
            ...
            con.setRequestProperty("Content-Type","application/json;charset=utf-8");
            con.setDoOutput(true);//서버에 데이터를 보낼 때는 이 옵션을 준다(POST, PUT)
            buffw=new BufferedWriter(new OutputStreamWriter(con.getOutputStream(),"UTF-8"));
            String jsonString=gson.toJson(board);
            ...
            mainActivity.myWebSocketClient.sendMsg(socketMessage); //쓰레드로 가능? 안되면 핸들러로
