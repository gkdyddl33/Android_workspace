...
public class ChatFragment extends Fragment {
    Socket socket;
    Thread thread; //네트워크 접속을 위한 쓰레드(메인쓰레드는 접속, 대기 ,루프 등에 빠지게 해서는 안 됨)
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        Button bt_send=(Button)view.findViewById(R.id.bt_send);
        ...
        thread=new Thread(){
            public void run() {
                try {
                    ...
                    chatThread=new ChatThread(socket, chatFragment);
                    chatThread.start();
                } catch (IOException e) {
                    e.printStackTrace();
                }
